...
public:
  ...
  VarToken create_q(const TString & index, const TString & exponent);
  VarToken create_Q(const TString & index, const TString & exponent);
  Term create_qprod(float coefficient, const vector<TString> & indices, const vector<TString> & exponents);
  Term create_Qprod(float coefficient, const vector<TString> & indices, const vector<TString> & exponents);

  Sum calculate_q2Avg(const vector<TString> & indices, int option=1);
  Sum calculate_q3Avg(const vector<TString> & indices, int option=1);
  Sum calculate_q4Avg(const vector<TString> & indices, int option=1);
  Sum calculate_q5Avg(const vector<TString> & indices, int option=1);
  Sum calculate_q6Avg(const vector<TString> & indices, int option=1);
  Sum calculate_q7Avg(const vector<TString> & indices, int option=1);
  Sum calculate_q8Avg(const vector<TString> & indices, int option=1);
  Sum calculate_qn(int n, int option=1);
};

TString QGenerators::emptyString = TString();
TString QGenerators::nString = TString("n");

VarToken QGenerators::create_q(const TString & index, const TString & exponent)
{
  return VarToken("q", index, exponent);
}
...
Term QGenerators::create_qprod(float coefficient, const vector<TString> & indices, const vector<TString> & exponents)
{
  Term result;
  result.setCoefficient(coefficient);
  for (unsigned int k=0; k<indices.size(); k++)
    {
    result.addVarToken(create_q(indices[k],exponents[k]));
    }
  return result;
}
...
Sum QGenerators::calculate_q2Avg(const vector<TString> & indices, int option)
{
  Sum result;
  vector<TString>  exponents;
  for (unsigned int k=0; k<2;k++) exponents.push_back(emptyString);
  result.addTerm(create_Qprod(-1.0,generateN(indices),exponents));
  result.addTerm(create_Qprod(1.0,indices,exponents));
  if (option>0) result.regroup(option);
  return result;
}

Sum QGenerators::calculate_q3Avg(const vector<TString> & indices, int option)
{
  Sum result;
  vector<TString>  exponents;
  for (unsigned int k=0; k<uint(3);k++) exponents.push_back(emptyString);
  result.addTerm(create_Qprod(2.0,generateN(indices),exponents));
  vector<vector<TString>> perms21 = generate21(indices);
  result.addTerm(create_Qprod(-1.0,perms21[0],exponents));
  result.addTerm(create_Qprod(-1.0,perms21[1],exponents));
  result.addTerm(create_Qprod(-1.0,perms21[2],exponents));
  result.addTerm(create_Qprod(1.0,indices,exponents));
  if (option>0) result.regroup(option);
  return result;
}
...
Sum QGenerators::calculate_q4Avg(const vector<TString> & indices, int option)
{
  Sum result;
  Sum partialSum;
  vector<TString>  exponents;
  for (unsigned int k=0; k<4;k++) exponents.push_back(emptyString);
  result.addTerm(create_Qprod(-1.0,generateN(indices),exponents));
  vector<vector<TString>> perms = generate31(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q2Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  ...
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q3Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  result.addTerm(create_Qprod(1.0,indices,exponents));
//  cout  << endl << "    -- 4 -- " << endl;
//  cout << result.createLatex(1);
  if (option>0) result.regroup(option);
  //cout  << endl << "    -- 4 regrouped -- " << endl;

  return result;
}

Sum QGenerators::calculate_q5Avg(const vector<TString> & indices, int option)
{
  Sum result;
  Sum partialSum;
  vector<TString>  exponents;
  for (unsigned int k=0; k<5;k++) exponents.push_back(emptyString);
  result.addTerm(create_Qprod(-1.0,generateN(indices),exponents));
  vector<vector<TString>> perms = generate41(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q2Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate32(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q2Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate311(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q3Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate221(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q3Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate2111(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q4Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  result.addTerm(create_Qprod(1.0,indices,exponents));
  if (option>0) result.regroup(option);
...
Sum QGenerators::calculate_q6Avg(const vector<TString> & indices, int option)
{
  Sum result;
  Sum partialSum;
  vector<TString>  exponents;
  for (unsigned int k=0; k<6;k++) exponents.push_back(emptyString);
  result.addTerm(create_Qprod(-1.0,generateN(indices),exponents));
  vector<vector<TString>> perms = generate51(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q2Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate42(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q2Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate33(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q2Avg(perms[k]);
    result.addSum(-1.0, partialSum);
  ...
  perms = generate411(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q3Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate321(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q3Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate222(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q3Avg(perms[k]);
    result.addSum(-1.0, partialSum);
  ...
  perms = generate3111(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q4Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate2211(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q4Avg(perms[k]);
    result.addSum(-1.0, partialSum);
  ...
  perms = generate21111(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q5Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  result.addTerm(create_Qprod(1.0,indices,exponents));
  if (option>0) result.regroup(option);
...
Sum QGenerators::calculate_q7Avg(const vector<TString> & indices, int option)
{
  Sum result;
  Sum partialSum;
  vector<TString>  exponents;
  for (unsigned int k=0; k<7;k++) exponents.push_back(emptyString);
  result.addTerm(create_Qprod(-1.0,generateN(indices),exponents));
  vector<vector<TString>> perms = generate61(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q2Avg(perms[k]);
    result.addSum(-1.0, partialSum);
  ...
  perms = generate52(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q2Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate43(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q2Avg(perms[k]);
    result.addSum(-1.0, partialSum);
  ...
  perms = generate511(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q3Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate421(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q3Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate331(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q3Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate322(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q3Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate4111(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q4Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate3211(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q4Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate2221(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q4Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate31111(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q5Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate22111(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q5Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate211111(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q6Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  result.addTerm(create_Qprod(1.0,indices,exponents));
  if (option>0) result.regroup(option);
...
Sum QGenerators::calculate_q8Avg(const vector<TString> & indices, int option)
{
  Sum result;
  Sum partialSum;
  vector<TString>  exponents;
  for (unsigned int k=0; k<8;k++) exponents.push_back(emptyString);
  result.addTerm(create_Qprod(-1.0,generateN(indices),exponents));
  vector<vector<TString>> perms = generate71(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q2Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate62(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q2Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate53(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q2Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate44(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q2Avg(perms[k]);
    result.addSum(-1.0, partialSum);
  ...
  perms = generate521(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q3Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate431(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q3Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate332(indices);
//  printStringVectors(perms);
//  cout << " 332: size is " << perms.size() << endl;
//  exit(1);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q3Avg(perms[k]);
  ...
  perms = generate5111(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q4Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate4211(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q4Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate3311(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q4Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate2222(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q4Avg(perms[k]);
    result.addSum(-1.0, partialSum);
  ...
  perms = generate41111(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q5Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate32111(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q5Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate22211(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q5Avg(perms[k]);
    result.addSum(-1.0, partialSum);
  ...
  perms = generate311111(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q6Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate221111(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q6Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  perms = generate2111111(indices);
  for (unsigned int k=0; k<perms.size();k++)
    {
    partialSum = calculate_q7Avg(perms[k]);
    result.addSum(-1.0, partialSum);
    }
  result.addTerm(create_Qprod(1.0,indices,exponents));
  if (option>0) result.regroup(option);
...
Sum QGenerators::calculate_qn(int n, int option)
{
  vector<TString>  indices;
  TString s;
  for (unsigned int k=0; k< uint(n); k++)
    {
    s = nString; s += k+1;
    indices.push_back(s);
    }
  switch (n)
    {
      ...
      break;
      case 2: return calculate_q2Avg(indices,option); break;
      case 3: return calculate_q3Avg(indices,option); break;
      case 4: return calculate_q4Avg(indices,option); break;
      case 5: return calculate_q5Avg(indices,option); break;
      case 6: return calculate_q6Avg(indices,option); break;
      case 7: return calculate_q7Avg(indices,option); break;
      case 8: return calculate_q8Avg(indices,option); break;
    }
  return Sum();
}


void MomentCalculator(int n=2, int option=2)
{
  QGenerators gen;
  Sum result = gen.calculate_qn(n, option);
